...
export default function Sidebar() {
  ...
  return (
    <aside className="w-64 bg-[#f8f9fa] border-r border-[#dddddd] flex-col hidden md:flex h-screen sticky top-0 z-20 shadow-[1px_0_5px_rgba(0,0,0,0.05)]">
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center gap-3 px-4 py-2.5 rounded text-sm font-bold transition-colors group border
                ${isActive
      ...
      <div className="p-4 border-t border-[#dddddd] bg-white">
        <button
          ...
          className="flex items-center gap-3 text-[#a94442] hover:bg-[#f2dede] hover:border-[#ebccd1] border border-transparent w-full px-4 py-2.5 rounded text-sm font-bold transition-colors group disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {isLoggingOut ? (
